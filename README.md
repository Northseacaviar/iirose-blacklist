...
5. **（2026-09-26 增补）屏蔽信箱通知** —— 被拉黑者的**点赞/关注/点踩/转账**在侧栏「信箱」里的通知卡片不再出现
   ...
   - 信箱帧与信箱卡片里**都没有 uid**（只有名字与头像链接），所以这一类只能按「名字或头像」认人；**同名误伤接受**（不做严格模式），头像链接按「末段指纹」归一后判等（协议里是 `cartoon/600264`、界面上是 `.../cartoon/600264.jpg`）
...
- 收包字符串按前缀分类：
  ...
  - `@`  → **信箱通知帧（v0.3.0 新增）**：从第 3 个字符起解析，记录以 `<` 分隔、字段以 `>` 分隔；
    ...
    标记 `'^` 关注 / `'*` 点赞 / `'h` 点踩 / `'$` 转账（打赏）。**信箱帧没有 uid** —— 只能按名字/头像认人
  - 其余（`%` 快照、`~`、`&1{...}` 媒体事件等）原样透传
...
- **界面层**：侧栏信箱面板 `#leaveMsgHolder` 下的 `.cardTag` 条目按 名字/头像 命中后 `display:none`（不删节点、记住原 `display` 可还原；与原「隐藏私聊会话项」同一路子）。历史卡片跟 `conf.keepHistory` 走：关=新旧一起隐；开=面板首批渲染算历史留着，之后新插入的那条隐。
- **头像在哪：** 拉黑时会把该用户已知的头像链接一起快照进名单（`store.uids[uid].avatar`，来自房间/私聊/弹幕帧的头像字段），解除拉黑不受影响；老落盘没有这个字段也能读（缺省空串）。
- **误伤口径**：名字（trim+小写完全相等）或头像指纹任一命中即算命中；同名误伤 **接受**（北海 2026-09-26 拍板，不做严格模式、不加统计行）。
...
- 截至 2026-09-26 11:45（北京时间）：估算花费 **$1.5549**（≈11 元人民币）· 消息 651 · 工具调用 345
- 结构：主开发会话 $1.35 ／ 子 agent 独立审查 $0.17 ／ 部分相关折算 $0.04（明细见 [`docs/成本账.md`](docs/成本账.md)）
...
- 插件 v0.3.0（2026-09-26，需求：北海）：**屏蔽信箱通知** —— 被拉黑者的点赞/关注/点踩/转账在侧栏「信箱」里的通知卡片不再出现。
  ...
  - **实现**：新增 `filterMailFrame()`（逐记录判定，**认不出的记录字节原样保留**；只丢"形状完全认得出 + 命中名单"的那条，全丢完才整帧丢弃）、`mailRecordInfo()`、`avatarKey()`、`mailHit()`；界面层新增 `sweepMailCards()`（`#leaveMsgHolder .cardTag` → `display:none`，记住原 `display` 可还原），接进 `sweepAll()` 与 MutationObserver 的增量路径（新插入的卡片按 `.cardTag` 匹配自身，`querySelector` 不匹配自身这点踩过）、解除拉黑/关掉屏蔽时还原；`_diag.mailCards()` 逐条报告认人结果（哪个名字/头像指纹、命中谁、为什么没隐），`_diag.sweepMail()` 手动全扫。
