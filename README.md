...
- 收包字符串按前缀分类：
  ...
  - `@`  → **信箱通知帧（v0.3.0 新增）**：从第 3 个字符起解析，记录以 `<` 分隔、字段以 `>` 分隔；
    3 字段 = 房间公告（站级，不带人名，**一律不动**）；用户通知**格子数不固定**（官方样本 7 格：用户名`>`头像`>`性别`>`标记`>`背景`>`时间`>`颜色；**真机转账帧只有 6 格、标记落在第 3 格**），
    所以解析**按特征找、不认死下标**（见下）；标记 `'^` 关注 / `'*` 点赞 / `'h` 点踩 / `'$` 转账（打赏）。**信箱帧没有 uid** —— 只能按名字认人
  - 其余（`%` 快照、`~`、`&1{...}` 媒体事件等）原样透传
...
- **协议层**：`@` 帧逐记录判定，只丢「形状认得出 + 命中名单」的那一条，认不出的记录一律字节原样保留（零影响优先）。形状判定**按特征找、不认死下标**（v0.3.7 真机修正）：名字=第 0 格、标记=**第 1~5 格里第一个以 `'` 开头的格子**、整条记录必须同时出现 9~11 位数字（时间戳）与 6 位 hex（颜色），格子数 4~9；超出范围或凑不齐特征 = 形状不认识 → 放行。点赞/关注/点踩丢帧（红点不亮）；**转账永不丢帧**（钱优先）。
...
3. **信箱真机取证：已完成（2026-09-26）** —— 真机抓到转账帧 `@*Night cruise><头像URL>'$1>>1790402429>ffffef`，证实**格子数与标记位置都跟官方样本不同**（样本 7 格、标记在 `p[3]`；真帧 6 格、标记在第 3 格），解析已改为按特征找（v0.3.7）。真机抓帧工具：`docs/探针-信箱3-真帧.js`；日常排障用面板「调试日志 → 诊断」窗口。
