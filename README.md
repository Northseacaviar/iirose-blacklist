...
① 私聊屏蔽（无消息、无红点、无提示音）② 房间消息屏蔽（看不到 TA 的头像与发言）③ **弹幕屏蔽**（被拉黑者发的弹幕整帧丢掉，不显示）④ 信箱通知屏蔽（点赞/关注/点踩不产生、转账卡片不显示）⑤ **房间屏蔽**（房间列表里隐藏指定房间的卡片）⑥ 拉黑/解除随时可做，名单本地保存 ⑦ 拉黑时**默认连历史消息与点播卡片一起清掉**，可用面板开关改成「只拦新消息、旧记录留着」。

四条硬口径：

- **判据分两类**：房间消息（`[8]`）、私聊（`[1]`）、**弹幕（`[7]`）**帧里都有 uid，**按 uid 精确判**（帧里那个 uid 被拉黑 → 这条记录丢掉；单条弹幕帧即整帧丢弃）；只有**信箱帧与历史卡片没有 uid**，那两类才按名字（trim + 小写完全相等）认人。按头像判人已整块移除 —— 预置卡通头像（如 `cartoon/600264`）可能多用户共用，会连累无关路人。
